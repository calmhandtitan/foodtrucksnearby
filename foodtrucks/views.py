...
def findFoodTrucksbyLocation(latitude, longitude, radius, limit):
	...
	lon_offset = Decimal(geopy.units.degrees(arcminutes=geopy.units.nautical(miles=float(radius)/math.cos(int(latitude)))))
	...
	foodtrucks = list(Foodtruck.objects.filter(latitude__range=(south, north)).filter(longitude__range=(east, west)))
	
	# remove trucks that are too far
	for truck in list(foodtrucks):
		dist = distance((truck.latitude, truck.longitude), (float(latitude), float(longitude)) ).miles
		if dist > radius:
